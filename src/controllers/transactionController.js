  ...
  createTransaction: async (req, res) => {
    ...
    if (!customerId || !items || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json({
        message: "Customer ID and transaction items are required",
      });
    }
    ...
    try {
      ...
      for (const item of items) {
        ...
        // Simpan seluruh data produk dan quantity agar bisa dipakai nanti
        processedItems.push({
          product,
      ...
      for (const item of processedItems) {
        const { product, quantity, pricePerItem } = item;
        ...
        // Update semua kolom produk tanpa null
